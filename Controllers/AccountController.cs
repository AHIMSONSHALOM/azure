...
namespace ProductHub_MVC.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> GoogleCallback()
        {
            ...
            {
                TempData["ErrorMessage"] = "❌ Failed to retrieve email from Google profile.";
                return RedirectToAction("Login");
            }
        ...
        private async Task<IActionResult> ProcessGoogleLoginAsync(string email, string name, string? picture = null)
        {
            ...
            if (profiles.Count == 0)
            {
                LogActivity("Unknown", "GOOGLE_LOGIN_FAILED", $"Attempted Google login with unregistered email: {email}");
                TempData["ErrorMessage"] = "❌ This email is not registered in the system. Please register and wait for administrator approval.";
                return RedirectToAction("Login");
            }

            int targetUserId = (int)profiles[0]["userId"];
            string targetUser = profiles[0]["username"].ToString() ?? "";
            int isApprovedVal = (int)profiles[0]["isApproved"];

            if (isApprovedVal != 1)
            {
                LogActivity(targetUser, "GOOGLE_LOGIN_FAILED", $"Attempted Google login but account is pending approval. User: {targetUser}, Email: {email}");
                TempData["ErrorMessage"] = "⏳ Access Pending: Your account is waiting for administrator approval.";
                return RedirectToAction("Login");
            }
        ...
        public async Task<IActionResult> SelectGoogleProfile(int userId)
        {
            ...
            {
                LogActivity(targetUser, "GOOGLE_LOGIN_FAILED", $"Attempted Google sign-in for profile choice but account is pending admin approval. User: {targetUser}, Email: {email}");
                TempData["Error"] = "⏳ Access Pending: Your account is waiting for administrator approval.";
                return RedirectToAction("Login");
            }
        }
